...
# PDF会同时解析文字与页图
# -----------------------------
def analyze_pdf_visual(pdf_bytes, model):
    if not OPENAI_API_KEY:
        raise RuntimeError("服务器未配置 OPENAI_API_KEY。")
...
def call_deepseek(model, prompt, thinking=True):
    if not DEEPSEEK_API_KEY:
        raise RuntimeError("服务器未配置 DEEPSEEK_API_KEY。")
...
def call_openai_writer(model, prompt):
    if not OPENAI_API_KEY:
        raise RuntimeError("服务器未配置 OPENAI_API_KEY。")
    client = OpenAI(api_key=OPENAI_API_KEY)
    response = client.responses.create(
...
with st.sidebar:
    st.header("运行状态")
    st.write("视觉识读：", "✅ 已配置" if OPENAI_API_KEY else "❌ 未配置")
    st.write("DeepSeek：", "✅ 已配置" if DEEPSEEK_API_KEY else "❌ 未配置")
    st.caption("API Key保存在服务器，不会显示在浏览器页面。")
    ...
    if writer_provider == "DeepSeek":
        live_models = get_deepseek_models()
        model_options = live_models or [
...
if st.button("🚀 开始生成最终教学设计", type="primary", use_container_width=True):
    if not OPENAI_API_KEY:
        st.error("服务器尚未配置OpenAI API Key，无法进行PPT视觉识读。")
        st.stop()
    ...
    try:
        ...
        vision_report = analyze_pdf_visual(pdf_bytes, vision_model)
        ...
        if writer_provider == "DeepSeek":
            raw = call_deepseek(writer_model, writer_prompt, thinking)
        else:
            raw = call_openai_writer(writer_model, writer_prompt)
